...
Tipsboard ではこの処理をすべてローカルで行います。検索対象は選択中の vault にある `pages/*.md` だけです。画像、添付ファイル、KANBAN の JSON、pins の JSON などは検索対象に含めません。外部の検索 API やクラウド推論にも送信しません。
...
索引作成時は、vault の `pages/` 直下にある Markdown ファイルだけを読みます。Tipsboard のノートはフラットな `pages/*.md` として扱うため、サブフォルダ配下の Markdown は対象にしません。
...
embedding に渡すテキストは、本文だけではありません。タイトルと見出し階層も一緒に入れます。本文だけでは何の話題か分かりにくい短い段落でも、タイトルや見出しを足すことで検索クエリと結びつきやすくするためです。
...
`dense` は既定のモードです。クエリベクトルとチャンクベクトルのコサイン類似度を主なスコアとして使います。

このモードは、単語が完全一致していなくても意味が近い文書を拾えるのが利点です。一方で、固有名詞、短い記号、ファイル名に近い語など、字面の一致が重要な検索では取りこぼすことがあります。

### 3.2 `hybrid` モード

`hybrid` は dense の意味検索に、BM25 のキーワード検索スコアを混ぜるモードです。BM25 は外部エンジンを使わず、検索のたびにメモリ上で全チャンクに対して計算します。BM25 用の統計は索引ファイルには保存していません。
...
```text
最終スコア = 検索本体のスコア * 0.85 + タイトル・見出しブースト * 0.15
```
...
ノートタイトルや見出しにクエリがそのまま含まれる場合は、少しだけ順位を上げます。

タイトルにクエリが含まれる場合はブースト値 1.0、チャンク見出しに含まれる場合は 0.75 です。どちらにも含まれない場合は 0 です。この値を最終スコアに 15% だけ混ぜます。

このブーストは、意味検索だけでは順位が下がりがちな「タイトルそのものを探している」ケースを補うためのものです。ただし、ブーストだけで順位が決まりすぎないよう、重みは小さめにしています。
...
**閉域（オプション）:** 利用者 PC から Hub / GitHub へ出したくないときは次をセットします。
...
キャッシュは「一度取った最大サイズ」が残ります。以前に 5,000 件で取った JSON があると、`--limit-docs 500` だけでは HF に行かず、読み込み後に 500 件へ切り詰めます（ログに `Loaded dataset cache with fetch limits` と出ます）。逆にキャッシュが 500 件しかないのに 5,000 件欲しいときは、キャッシュ削除か `--refresh-dataset` が必要です。索引のチャンク数は vault の全 `pages/*.md` から決まるため、文書数が多いと embedding だけで数時間かかることがあります。
...
評価はチャンク単位ではなく文書単位です。検索結果はチャンクとして返りますが、同じ文書から複数チャンクが出る可能性があります。そのため、`pages/...` から document ID に変換したあと、同じ document ID は最初の出現だけを採用します。
...
ただし、現時点では本番検索にも評価パイプラインにも reranking は組み込んでいません。`TIPSBOARD_SEMANTIC_EVAL_RERANKER` を指定しても、評価ログに「指定されたが本番経路には未接続」と出るだけです。導入する場合は、まず Transformers.js / ONNX での実行可否、速度、メモリ使用量、日本語性能を別途検証する必要があります。
