...
`package.json` の説明文どおり、機能の要約は「vault 上のノートグリッド、バックリンク付きエディタ、KANBAN、**Canvas（無限ボード）**、Mermaid・数式、画像、プレーンファイル、**添付ファイル一覧（Attachments ビュー）**、**セマンティック検索（ローカル、既定で有効）**」である。
...
| Vault I/O | `src/host/vault.ts` | スナップショット読み書き、ノート CRUD、画像インポート、JSON import/export。ノート改名・削除時に Canvas の note ノードもパッチ。 |
| Canvas（Host） | `src/host/canvas.ts` | `.tipsboard/canvas/*.canvas` の一覧・読み書き・作成・削除。JSON ドキュメントの sanitize、note ノードのパス追従・削除時クリーンアップ。 |
...
| Canvas（WebView） | `webview/src/components/canvas/` | `CanvasView`（キャンバス切替・保存 UI）、`CanvasBoard`（`@xyflow/react` ベースのボード）、ノード種別コンポーネント、`flowAdapter.ts` で Host JSON と Flow 状態を相互変換。 |
...
vault/
  ...
  .tipsboard/
    kanban.json
    pins.json
    canvas/              … ビジュアル Canvas（*.canvas JSON）
...
- **用途**: ビジュアルボード。ノートやテキスト、画像、リンク、グループをノードとして配置し、エッジで接続する。
- **ファイル**: `<name>.canvas`（JSON、`version: 1`）。`nodes`（位置・サイズ・種別）、`edges`（接続と side）、`viewport`（zoom / pan）を保持。
- **Host**: `src/host/canvas.ts` が sanitize・CRUD・note パス追従を担当。WebView は `@xyflow/react` で編集し RPC 経由で保存する。
...
- リネーム時、**KANBAN** の `note_path`、**pins** のパス、および **Canvas** の note ノード `path` を **パッチ**する。ノート削除時は Canvas から該当 note ノードと接続 edge を除去する。
...
### 9.12 Canvas UI（`CanvasView` / `CanvasBoard`）

- **保存形式**: vault 相対 `.tipsboard/canvas/<name>.canvas` の JSON（`version: 1`、`nodes`、`edges`、`viewport`）。Host 側 `sanitizeCanvasDocument` で未知フィールドや不正 edge を除去する。
- **ノード種別**: `text`（自由テキスト）、`note`（vault Markdown path、クリックでノートを開く）、`image`（`assets/images/...` 等の vault 相対 path）、`link`（外部 URL）、`group`（ラベル付き枠）。`parentId` で group 内にネスト可能。
- **エッジ**: ノード間を `fromSide` / `toSide`（`top` / `right` / `bottom` / `left`）で接続。React Flow 上でドラッグ接続・再接続・削除が可能。エッジ中点クリックで **ラベル**（`label`）を編集。始点・終点に **矢印**（`fromEnd` / `toEnd`: `none` | `arrow`）をトグル可能。
- **操作**: ノードのドラッグ・リサイズ、**Space+ドラッグ**でパン（`useCanvasPanMode`）、左下コントロールでズーム in/out・全体フィット・VS Code 全画面切替（`toggleFullScreen` RPC）。
- **永続化**: WebView 側で debounce 保存（`saveCanvas` RPC）。保存状態はツールバーに表示。
- **キャンバス管理**: ツールバーからキャンバス作成・切替・削除。`getSnapshot` の `canvases: CanvasSummary[]` で一覧を保持。
- **ノート連動**: ノート改名時 `patchCanvasNotePaths`、削除時 `cleanupCanvasAfterNoteDelete`、`readVault` 時に存在しない path の note ノードを `pruneAllCanvasNoteNodes` で除去。
