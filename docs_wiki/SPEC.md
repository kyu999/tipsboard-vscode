...
`onDidCreate` / `onDidChange` / `onDidDelete` を **約 250ms debounce** し、まとめて次を送る。

`{ source: "tipsboard-vscode-host", kind: "event", event: "vault-files-changed" }`

WebView の `App.tsx` の挙動:

- **未保存がない**（`saveState` が `idle` / `saved`）→ すぐ `getSnapshot()` で再同期。
- **未保存がある**（`unsaved` / `saving` / `error`）→ 自動では上書きせず、バナーを出し「更新」相当のボタンで **既存の未保存破棄確認**のあと `getSnapshot()`。
- 選択中ノートが同一路径で残り、**ディスク上の本文が前回スナップショットと異なる**場合は `editorSessionId` を進め **`NoteEditor` を再マウント**して CodeMirror の初期 doc を差し替える。
...
| `event` | `vault-files-changed`（監視中のコアファイルがディスク上で変わったとき。debounce 後） |
