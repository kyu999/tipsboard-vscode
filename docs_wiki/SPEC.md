...
`TipsboardPanel` は vault root に対して `**/*.md` と `.tipsboard/kanban.json` / `.tipsboard/pins.json` の `FileSystemWatcher` を張る。外部エディタ・Git・同期ツールなどで変更された場合は §4.4.2 の `vault-files-changed` で WebView に通知し、未保存状態を尊重しながら `getSnapshot()` で再同期する。
