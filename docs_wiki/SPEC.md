...
`package.json` の説明文どおり、機能の要約は「vault 上のノートグリッド、バックリンク付きエディタ、KANBAN、Mermaid・数式、画像、プレーンファイル、**添付ファイル一覧（Attachments ビュー）**、**セマンティック検索（ローカル、既定で有効）**」である。
...
| セマンティック検索（Host） | `src/host/semantic.ts`, `semanticProviderFactory.ts`, `semanticSettings.ts` | `pages/*.md` をチャンク化し embedding・`.tipsboard/semantic/` への索引書き込み・検索。設定 `tipsboard-vscode.semanticSearch.*` と `ExtensionContext.globalStorageUri` 配下のモデルキャッシュ。 |
...
| グラフ・検索 | `webview/src/lib/noteIndex.ts`, `searchNotes` 等 | メモリ上のリンクグラフ、補完候補、タグ、**ヘッダーによるキーワード検索**。セマンティック検索は Host 側（`semantic.ts`）と RPC。 |
