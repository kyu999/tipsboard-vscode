...
multi-root workspace では、Tipsboard の対象を自動的には決めない。`tipsboard-vscode.vaultFolder` に vault として使う workspace folder 名を設定する。通常利用では、ノートやドキュメント用フォルダを VS Code で単一フォルダとして開く運用を推奨する。
...
新規ノートは、まず workspace root 直下の `inbox/` に作成する。
...
`inbox/` の場所は、初期仕様では workspace root 直下に固定でよい。将来的に必要になった場合は、inbox path を設定可能にする。

`inbox/` が存在しない場合は Tipsboard が作成する。`inbox` がファイル等で使えない場合は `Tipsboard inbox/` にフォールバックする。フォールバック先も衝突する場合は番号を付けて一意化する。
...
選択中ノートでは、本文をクエリにしたセマンティック検索結果をノート単位に集約し、近傍ノートとして Related 領域に表示する。近傍候補は、自分自身、既存のリンク関連、弱いスコアの候補を除外し、通常のノートカード内に一致度、ヒット見出し、本文スニペットを表示する。

内部リンクの接続がまだないノートを見つけやすくするため、発リンクも被リンクもないノートはリンク孤立として Related 領域に通知する。この判定はセマンティックな孤立ではなく、あくまでリンクグラフ上の孤立である。

---

## 9. AI Assisted Organization

Tipsboard は、workspace root の `inbox/` 内のノートに対して移動先候補を提案する。

初期実装では、提案対象は `inbox/` または Tipsboard が fallback として作成した `Tipsboard inbox/` 系フォルダ内の単一ノートに限定する。
移動先候補は、workspace root 配下にすでに存在し、Markdown ノートを含む既存フォルダのみとする。`inbox/`、`Tipsboard inbox/`、`.tipsboard/`、`.git/`、`node_modules/`、`dist/`、`build/`、`out/` は候補にしない。

提案には以下の signal を利用する。

- 内部リンク
- セマンティック検索による近傍ノート
- タグ
- タイトルパターン
- 既存フォルダ構造から作る軽量 folder profile

内部リンクは強い signal として扱う。たとえば対象ノートが `[OAuth]` を含み、リンク先タイトルのノートが `docs/auth/` 配下にある場合、`docs/auth/` を強く加点する。ただし `Overview` や `README` のような一般的なタイトル、または重複タイトルは弱く扱う。

セマンティック検索が有効な場合は、対象ノート本文に近い既存ノート top-k を見て、その所属フォルダを加点する。大きなフォルダが過剰に有利にならないよう、フォルダ内ノート数で軽く正規化する。

セマンティック検索が無効な場合でも、タグ、内部リンク、タイトルパターン、キーワード一致を使って候補を出す。ただし UI では、セマンティック検索を有効にすると候補精度が上がることを案内する。

タグは、既存 vault 内で特定フォルダに偏っているものを強く扱う。たとえば `#adr` の多くが `adr/` にある場合、`#adr` を含む未整理ノートでは `adr/` を加点する。`#todo` や `#memo` のように多くのフォルダに広がりやすいタグは弱く扱う。

タイトルパターンは、`ADR-001`、`Meeting 2026-05-24`、`RFC OAuth Refresh` など、既存フォルダ分布と結びつきやすい形式を補助 signal として扱う。

folder profile は、フォルダ配下のタイトル、見出し、タグ、リンク先タイトル、本文冒頭から軽量な語彙 profile を作り、対象ノートとのキーワード一致を補助 signal として使う。未整理ノートは profile の学習対象に含めない。
...
提案には理由と confidence を表示する。confidence は score の絶対値だけでなく、1位と2位の差、複数 signal が揃っているか、内部リンクやセマンティック近傍など強い signal があるかで決める。

confidence が低い場合でも候補は提示する。ただし、UI で信頼度が低いことを警告し、移動先を確認するよう促す。
...
初期仕様では、提案は workspace root の `inbox/` 内のノートに限定する。既存フォルダ内のノートを勝手に再配置候補として扱うことはしない。

UI では、`inbox/` 内のノートを開いている間だけ、ノート上部に控えめな inbox notice を表示する。
この notice は「このノートはまだ整理対象である」ことを伝えるためのもので、通常のピン留めや削除などのクイック操作とは分ける。
ユーザーが notice 内の提案操作を実行すると、候補フォルダを小さな popover で表示する。
...
通常の Markdown 相対リンクは移動によって壊れる可能性があるため、整理提案 UI では対象ノート内に相対 Markdown link がある場合に警告する。
初期実装では、相対 Markdown link の自動修正は行わない。必要に応じて Tipsboard link への変換機能を使う。

ファイル移動後、Tipsboard は workspace index を更新する。
移動先フォルダに同名ファイルがある場合は、`foo (2).md` のように空いているファイル名を自動で選ぶ。
...
### Phase 4: Inbox Organization Suggestions

- 新規ノートを workspace root の `inbox/` に作成する
- inbox 系フォルダ内ノートに移動先候補を出す
- 提案理由を表示する
- ユーザー承認時のみファイルを移動する
...
- multi-root workspace の自動判定（必要な場合は `tipsboard-vscode.vaultFolder` で明示する）
...
初期実装では、対象を「VS Code で開いている workspace folder」に寄せ、フォルダ選択 UI を持たないシンプルな体験を優先する。multi-root workspace では `tipsboard-vscode.vaultFolder` により対象 root を明示する。
