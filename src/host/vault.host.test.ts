...
import { createNote, deleteNote, extractTitle, importImageBuffers, readVault, saveNote, setNotePinned } from "./vault.js";
...
describe("vault (VS Code host)", () => {
  ...
  it("imports image buffers with uuid filenames", async () => {
    await withVault(async (vaultPath) => {
      ...
      const [a] = await importImageBuffers(vaultPath, [{ name: "animal_chara.png", data: pngBytes }]);
      const uuidFileRe =
